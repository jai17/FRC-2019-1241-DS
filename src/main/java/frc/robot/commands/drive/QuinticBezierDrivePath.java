...
public class QuinticBezierDrivePath extends Command {
	...
	protected void execute() {
		if (reverse) {
			...
			Robot.drive.regulatedDrivePID(-distance, curve.findAngle(counter), 1,  1, speed);
		} else {
			if (Robot.drive.getAveragePos() > curve.findHypotenuse(counter) && counter < curve.size())
				counter++;
	...
	protected boolean isFinished() {
		return Robot.drive.getAveragePos() == distance || Robot.drive.getAveragePos() >= distance||isTimedOut();
	}
